...
class ManageDoctor extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        //build data sang dạng react-select, khi chuyển ngôn ngữ sẽ ko lỗi
    ...
    handleChangeSelect = async (selectedOption) => {
        ...
        let res = await getDetailInfoDoctor(selectedOption.value.id);

        if (res && res.data.errCode === 0 && res.data.data && res.data.data.Markdown) {
            let markdown = res.data.data.Markdown;

            let addressClinic = '', nameClinic = '', note = '', specialtyId = '', clinicId = '',
                paymentId = '', priceId = '', provinceId = '', selectedPrice = '',
                selectedPayment = '', selectedProvince = '', selectedSpecialty = '', selectedClinic = '';
